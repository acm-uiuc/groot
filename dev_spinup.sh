...
forever -f groot-users-service/server.js  | tee log/dev/groot-users-service.log | sed -e 's/^/[groot-users-service] /' \
& forever -f groot-groups-service/server.js  | tee log/dev/groot-groups-service.log | sed -e 's/^/[groot-groups-service] /' \
& forever -f groot-desktop-frontend/server.js | tee log/dev/groot-desktop-frontend.log | sed -e 's/^/[groot-desktop-frontend] /' \
& forever -f groot-events-service/server.js  | tee log/dev/groot-events-service.log | sed -e 's/^/[groot-events-service] /' \
